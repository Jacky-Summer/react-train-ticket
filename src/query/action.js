...
export const ACTION_SET_CHECKED_ARRIVE_STATIONS = 'SET_CHECKED_ARRIVE_STATION'
...
export const ACTION_SET_ARRIVE_TIME_START = 'SET_ARRIVE_TIME_STAR'
...
export function toggleHighSpeed() {
  return (dispatch, getState) => {
    ...
    if (highSpeed) {
      dispatch(setHighSpeed(false))
      dispatch(setCheckedTrainTypes({}))
    } else {
      dispatch(setHighSpeed(true))
      let newTrainTypes = { ...checkedTrainTypes }
      newTrainTypes[1] = true
      newTrainTypes[5] = true
      dispatch(setCheckedTrainTypes(newTrainTypes))
    }
  }
}
...
// 后一天
export function nextDate(nextDate) {
  return (dispatch, getState) => {
...
// 前一天
export function prevDate(prevDate) {
  return (dispatch, getState) => {
    const { departDate } = getState()
